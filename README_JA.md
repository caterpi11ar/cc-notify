...
<details>
<summary><strong>macOSで「開けません」「開発元不明」と表示されます</strong></summary>

アプリはApple開発者証明書で署名されていないため、macOS Gatekeeperがインターネットからダウンロードした未署名アプリをブロックします。

**方法1** — アプリを右クリック →「開く」を選択 → ダイアログで「開く」をクリック（初回のみ）。

**方法2** — **システム設定 → プライバシーとセキュリティ**を開き、下にスクロールして **「このまま開く」** をクリック。
